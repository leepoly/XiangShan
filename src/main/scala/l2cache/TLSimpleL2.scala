...
class MetadataEntry(tagBits: Int) extends Bundle {
  val valid = Bool()
  val dirty = Bool()
  val tag = UInt(width = tagBits.W)
  val rr_state = Bool()
  // override def cloneType = new MetadataEntry(tagBits).asInstanceOf[this.type]
}
...
with HasL4CacheParameters
{
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val s_idle :: s_gather_write_data :: s_send_bresp :: s_update_meta :: s_tag_read_req :: s_tag_read_resp :: s_tag_read :: s_merge_put_data :: s_data_read :: s_data_write :: s_wait_ram_awready :: s_do_ram_write :: s_wait_ram_bresp :: s_wait_ram_arready :: s_do_ram_read :: s_data_resp :: Nil = Enum(16)
      ...
      val size_reg = Reg(UInt(width=in.a.bits.params.sizeBits.W))
      ...
      val (meta_array, omSRAM) = DescribedSRAM(
        ...
        size = nSets,
        data = Vec(nWays, new MetadataEntry(tagBits))
      )
      ...
      val curr_state_reg = Reg(Bits(width = nWays.W))
      ...
      val repl_way = Mux((curr_state_reg & curr_mask).orR, PriorityEncoder(curr_state_reg & curr_mask),
        Mux(curr_mask.orR, PriorityEncoder(curr_mask), 0.U))
      ...
      val rst_metadata = Wire(Vec(nWays, new MetadataEntry(tagBits)))
      for (i <- 0 until nWays) {
      ...
      when (state === s_tag_read) {
        ...
        log("set: %d hit: %d rw: %d update_way: %d curr_state: %x next_state: %x",
          idx, hit, ren, update_way, curr_state_reg, next_state)
      }

      val update_metadata = Wire(Vec(nWays, new MetadataEntry(tagBits)))
      for (i <- 0 until nWays) {
...
object TLSimpleL4Cache
{
  def apply()(implicit p: Parameters): TLNode =
  {
    if (p(NL4CacheCapacity) != 0) {
      ...
      val tlsimpleL4cache = LazyModule(new TLBuffer(BufferParams.none))
      tlsimpleL4cache.node
    }
  }
}
